 ...
 * that a Valve somewhere in the pipeline (usually the last one) should process
 * the request and create the corresponding response, rather than trying to pass
 * the request on.
 * 
 * There is generally a single Pipeline instance is associated with a Container.
 * The container's normal request processing functionality is encapsulated in a
 * container-specific Valve, which would always be executed at the end of the
 * pipeline. To facilitate this, the <code>setBasic()</code> method is provided
 * to set the Valve instance that will always be executed last. Other Valves
 * will be executed in the order that they were added, before the basic Valve is
 * executed.
 * 
 * @author J. Godara
 */
